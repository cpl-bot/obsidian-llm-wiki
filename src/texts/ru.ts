...
export const RU_TEXTS = {
    ...
    removedProviderResetNotice: 'В этом хранилище был настроен провайдер, которого больше нет в этой сборке. Провайдер сброшен на значение по умолчанию, а сохранённые облачные учётные данные удалены — выберите провайдера и введите ключ в настройках.',
    ...
    taskPoliciesDesc: 'По одной записи на шаг конвейера, `шаг=режим:рассуждение`, через запятую — например `extract=text:off`. Режимы: `-` (без изменений), `schema`, `json`, `text`; рассуждение: `-`, `off`, `on`, `low`, `medium`, `high`. Имена шагов включают extract, extract-retry, lemma-classify, merge-triage, dedup, page-generate, related-page, lint-dedup (имя с опечаткой не совпадёт ни с чем). Пусто — встроенная основа (извлечение в текстовом режиме). Нечитаемая запись не сохраняется.',
    ...
    forcePdfSupportDesc: 'Выкл по умолчанию. Включите, если ваш провайдер не указан как нативный, но всё же может обрабатывать PDF-файлы. Когда вкл, PDF будет отправлен вашему текущему провайдеру — если будет отклонён, вы увидите чёткое уведомление. Нативным PDF-провайдерам (Anthropic / OpenAI) это не нужно.',
