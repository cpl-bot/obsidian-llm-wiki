export const KO_TEXTS = {
    ...
    removedOAuthProviderNotice: '이 빌드에서는 ChatGPT 구독 로그인 공급자가 제거되었습니다. 공급자가 초기화되고 저장된 자격 증명도 삭제되었습니다. 설정에서 공급자와 모델을 다시 선택하세요.',
    removedProviderResetNotice: '이 보관함에 설정된 Provider는 이 빌드에 더 이상 포함되지 않습니다. Provider를 기본값으로 재설정하고 저장된 클라우드 자격 증명을 삭제했습니다 — 설정에서 Provider와 키를 다시 지정하세요.',
    ...
    forcePdfSupportDesc: '기본값은 꺼짐. provider가 네이티브 지원 목록에 없지만 PDF를 처리할 수 있다면, 이 스위치를 켜서 시도해 볼 수 있습니다. 켜면 PDF가 현재 provider로 전송되며, 거부되면 알림으로 안내됩니다. 네이티브 PDF provider(Anthropic / OpenAI)는 이 설정이 필요 없습니다.',
