export const ZH_TEXTS = {
    ...
    removedOAuthProviderNotice: '本版本已移除 ChatGPT 订阅登录 Provider；Provider 已重置，其存储的凭据已清除。请在设置中重新选择 Provider 和模型。',
    removedProviderResetNotice: '此库中配置的 Provider 已不再包含在本版本中。Provider 已重置为默认值，其存储的云凭据已被清除 —— 请在设置中重新选择 Provider 并填写密钥。',
    unsupportedPlatform: 'LLM Wiki 不支持 Windows，插件未加载。',
    ...
    forcePdfSupportDesc: '默认关闭。如果你的 provider 不是原生支持 PDF 的，但又确实能处理 PDF，可以打开这个开关试一试。打开后会发送 PDF 给当前 provider，如果被拒绝，会用提示告诉你。原生支持 PDF 的 provider（Anthropic / OpenAI）不需要这个开关。',
