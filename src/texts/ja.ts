export const JA_TEXTS = {
    ...
    removedOAuthProviderNotice: 'このビルドでは ChatGPT サブスクリプションのサインイン プロバイダーを削除しました。プロバイダーはリセットされ、保存されていた資格情報も消去されました。設定でプロバイダーとモデルを選び直してください。',
    ...
    forcePdfSupportDesc: 'デフォルトはオフ。プロバイダーがネイティブ対応リストにないものの PDF を処理できる場合は、このスイッチをオンにして試すことができます。オンにすると PDF が現在のプロバイダーに送られ、拒否された場合は通知で案内されます。ネイティブ PDF プロバイダー（Anthropic / OpenAI）では不要です。',
