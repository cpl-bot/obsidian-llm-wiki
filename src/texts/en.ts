export const EN_TEXTS = {
    ...
    removedProviderResetNotice: 'A provider that this build no longer includes was configured in this vault. The provider has been reset to the default and its stored cloud credentials were cleared — pick a provider and enter a key in Settings.',
    ...
    forcePdfSupportDesc: 'Off by default. Turn this on if your provider isn\'t listed as native but can still handle PDF files. When on, the PDF will be sent to your current provider — if it gets rejected, you\'ll see a clear notice. Native PDF providers (Anthropic / OpenAI) don\'t need this.',
