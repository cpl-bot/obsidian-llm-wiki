export const PT_TEXTS = {
    ...
    removedOAuthProviderNotice: 'O provedor de login por assinatura do ChatGPT foi removido nesta build; o provedor foi redefinido e as credenciais armazenadas foram apagadas. Escolha um provedor e um modelo nas Configurações.',
    removedProviderResetNotice: 'Neste cofre estava configurado um provedor que esta versão já não inclui. O provedor foi redefinido para o padrão e as credenciais cloud armazenadas foram apagadas — escolha um provedor e insira uma chave nas Configurações.',
    ...
    strictEgressName: 'Saída de rede estrita (recomendado)',
    strictEgressDesc: 'Permite pedidos de saída apenas para os hosts de fornecedores integrados, a Base URL que configurou e localhost. Desative apenas para um proxy ou gateway corporativo.',
    strictEgressWarning: 'A saída de rede estrita está DESLIGADA. O plugin pode enviar a sua chave de API para qualquer host HTTPS para o qual uma Base URL aponte.',
    egressDeniedNotice: 'Pedido para "{host}" bloqueado pela política de saída: {reason}',
    egressBaseUrlRejected: 'Base URL rejeitada: {reason}',
    egressReasonInvalidUrl: 'não foi possível analisar o endereço',
    egressReasonScheme: 'apenas https:// é permitido (http:// só para localhost)',
    egressReasonUserinfo: 'o endereço inclui credenciais (user:pass@host)',
    egressReasonPrivateAddress: 'o endereço aponta para uma rede privada ou link-local',
    egressReasonHostNotAllowed: 'o host não está na lista de saída permitida',
    ...
    sourceRejectedPdfUnsupported: '⏭️ «{filename}» ignorado — o seu provedor ou modelo atual não aceita PDF. Troque de provedor ou ative «Forçar suporte a PDF» em Settings → LLM Configuration → Advanced e tente novamente.',
    ...
    forcePdfSupportDesc: 'Desativado por padrão. Ative esta opção se o seu provedor não está na lista nativa, mas mesmo assim consegue processar PDFs. Ao ativar, o PDF será enviado ao seu provedor atual; se for rejeitado, você verá um aviso. Provedores PDF nativos (Anthropic / OpenAI) não precisam desta opção.',
    ...
    taskPoliciesDesc: 'Uma entrada por etapa do pipeline, `etapa=modo:raciocinio`, separadas por vírgulas — p. ex. `extract=text:off`. Modos: `-` (inalterado), `schema`, `json`, `text`; raciocínio: `-`, `off`, `on`, `low`, `medium`, `high`. Os nomes de etapa incluem extract, extract-retry, lemma-classify, merge-triage, dedup, page-generate, related-page, lint-dedup (um nome mal escrito não corresponde a nada). Deixe em branco para a base integrada (extração em modo texto). Uma entrada ilegível não é guardada.',
