export const ZH_HANT_TEXTS = {
    ...
    removedOAuthProviderNotice: '本版本已移除 ChatGPT 訂閱登入 Provider；Provider 已重設，其儲存的憑證已清除。請在設定中重新選擇 Provider 與模型。',
    removedProviderResetNotice: '此庫中設定的 Provider 已不再包含於本版本。Provider 已重設為預設值，其儲存的雲端憑證已被清除 —— 請在設定中重新選擇 Provider 並填寫金鑰。',
    ...
    forcePdfSupportDesc: '預設關閉。如果你的 provider 不在原生支援清單中，但其實可以處理 PDF，可以打開這個開關試試。打開後 PDF 會送給目前的 provider，若被拒絕，會用提示通知你。原生支援 PDF 的 provider（Anthropic / OpenAI）不需要這個開關。',
