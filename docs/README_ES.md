...
- docs: README.md + docs/README_<locale>.md (11 idiomas) + docs/MODEL-GUIDE.md
- first-published: 2025-09 (v0.1.0)
- latest: v1.27.0 (MINOR — citas de página fuente, gate de candidatos, UI taskPolicies, Fix Dead Links leave-it; 36 commits, 3677 tests)
...
**Puntuación perfecta en la revisión de Obsidian • Recuperación por grafo sin embeddings • 11 idiomas nativos • Ingesta nativa de PDF • Funciona con cualquier proveedor • Local primero • Sin backend • Compatible con RGPD**
...
| **Proveedores LLM** | ✅ 16+ (Anthropic, OpenAI, Gemini, DeepSeek, Qwen, Grok, Kimi, GLM, MiniMax, Step, Hunyuan, MiMo, Gemma, Ollama, LM Studio, OpenRouter, Anthropic-Compatible, ...) | 🟡 Compatible con OpenAI | 🟡 Suscripción vía Claude Code | 🟡 Suscripción vía Claude Code / Codex | 🟡 Compatible con OpenAI |
...
- **🔌 Cualquier modelo por el que ya pagas.** Anthropic, OpenAI, Gemini, DeepSeek, Qwen, Grok, Kimi, GLM, MiniMax, Step, Hunyuan, MiMo, Gemma, Ollama, LM Studio, OpenRouter, Anthropic-compatible, endpoint personalizado — más de dieciséis proveedores, ninguno requiere tener un endpoint de embeddings.
...
- **Necesitas RAG sobre grandes corpus externos** (Confluence, Notion, arXiv, páginas web scrapeadas) — el plugin ingiere tu vault además de archivos PDF independientes; el RAG masivo sobre corpus externos está fuera del alcance por diseño.
...
2. **Configura un proveedor.** Abre Configuración → Karpathy LLM Wiki → elige un proveedor (OpenAI, Anthropic, Ollama, etc.) → introduce la clave API (no necesaria para local) → haz clic en **Test Connection** → Guardar.
...
Tres rutas de entrada, conmutable por ingesta:

1. **☁️ Proveedores cloud con PDF nativo** — Anthropic, OpenAI y Google Gemini leen PDFs como file parts de fábrica. Sin configuración más allá de seleccionar el proveedor.
2. **🖥️ OCR local en Apple Silicon** — [oMLX](https://github.com/jundot/omlx) integra Microsoft Markitdown como backend PDF→Markdown integrado. Activa Markitdown en oMLX, carga [Baidu Unlimited-OCR](https://huggingface.co/baidu/Unlimited-OCR) (3B / 570M activos, open-source 2026-06) como modelo de visión, apunta el plugin a oMLX como proveedor Custom OpenAI-Compatible, activa **Force PDF Support** y elige el modelo multimodal que oMLX está sirviendo. El PDF nunca sale de tu máquina.
3. **🔌 Force PDF Support** — para cualquier otro endpoint OpenAI/Anthropic-compatible que acepte file parts, el plugin intenta la llamada (Configuración → Configuración LLM → Avanzado). El endpoint decide; los errores se muestran como Notice localizada.
...
Rechazamos deliberadamente la ruta de embeddings en [Issue #175](https://github.com/green-dalii/obsidian-llm-wiki/issues/175). La señal del grafo ya está ahí — cada `[[wiki-link]]` es una arista "estos están relacionados" curada a mano, y la mayoría de los proveedores que soportamos (Ollama, LM Studio, Anthropic, Kimi, GLM, MiniMax) no tienen ningún endpoint `/v1/embeddings`. Añadir un modelo de embeddings significaría una descarga por página, un adaptador por proveedor y cero beneficio en la calidad de recuperación.
...
Para ingesta de PDF, consulta [Ingesta de Documentos / PDF / Imágenes](#-ingesta-de-documentos--pdf--imágenes) en Características — Anthropic, OpenAI, Gemini leen PDFs como file parts de forma nativa; **Force PDF Support** cubre cualquier otro endpoint que acepte file parts.

> 📖 **Tabla completa de selección** (cloud + local + OCR PDF + cuantización + niveles de hardware) → [docs/MODEL-GUIDE.md](https://github.com/green-dalii/obsidian-llm-wiki/blob/main/docs/MODEL-GUIDE.md)
...
✅ Sí. Anthropic, OpenAI, Gemini leen PDFs de forma nativa; **Force PDF Support** cubre cualquier otro endpoint que acepte file parts, y en Apple Silicon la conversión puede ejecutarse íntegramente en local. Las tres rutas están en [Ingesta de Documentos / PDF / Imágenes](#-ingesta-de-documentos--pdf--imágenes). Las imágenes y los documentos Office no se ingieren directamente: conviértelos antes a Markdown o PDF.
